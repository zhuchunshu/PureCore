  ...
  <template v-if="showUserAuth">
    ...
    <div v-if="isLoggedIn()" class="dropdown dropdown-end">
      <label tabindex="0" class="btn btn-ghost btn-sm hover:bg-primary/10 gap-2">
        <div class="w-6 h-6 rounded-full bg-gradient-to-br from-blue-400 to-emerald-400 flex items-center justify-center text-xs font-bold text-white">
      ...
      <ul tabindex="0" class="menu menu-sm dropdown-content mt-2 z-50 p-2 shadow-xl shadow-primary/10 bg-base-100/95 backdrop-blur-xl rounded-box w-48 border border-primary/20">
        <li><RouterLink to="/dashboard" class="hover:text-primary hover:bg-primary/5 rounded-lg transition-colors">📊 {{ t('user.dashboard') }}</RouterLink></li>
        <li><button @click="clearTokens(); router.push('/')" class="hover:text-error hover:bg-error/5 rounded-lg transition-colors w-full text-left">🚪 {{ t('admin.logout') }}</button></li>
      </ul>
    </div>
